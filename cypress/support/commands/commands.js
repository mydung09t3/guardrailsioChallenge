 ...
 */
Cypress.Commands.add('login', (email, passWord) => {
  cy.get('.mat-button-wrapper > span:contains(Account)')
    .should('be.visible')
    .click();
...
Cypress.Commands.add(
  'fillAddress',
  (country, name, mobilePhone, zipCode, address, city, state) => {
    ...
    cy.get('input[data-placeholder="Please provide a mobile number."]').type(
      mobilePhone,
    );
    cy.get('input[data-placeholder="Please provide a ZIP code."]').type(
      zipCode,
    );
